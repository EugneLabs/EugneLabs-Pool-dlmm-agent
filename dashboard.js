...
function handleLogs(req, res) {
  ...
  const wallet = url.searchParams.get("wallet") || "";
  const offset = parseInt(url.searchParams.get("offset") || "-1", 10); // -1 = tail mode
  const today = new Date().toISOString().slice(0, 10);
  const logFile = path.resolve(`./logs/agent-${today}.log`);
  try {
    if (!fs.existsSync(logFile)) return jsonReply(res, 200, { ok: true, lines: [], total: 0, nextOffset: 0 });
    const raw = fs.readFileSync(logFile, "utf8");
    const all = raw.split("\n").filter(Boolean);
    let filtered = all;
    if (wallet && wallet !== "all") {
      const addr = wallet.toLowerCase().slice(0, 12);
    ...
    let slice, nextOffset;
    if (offset >= 0) {
      // incremental mode: return only lines since offset
      slice = filtered.slice(offset);
      nextOffset = total;
    } else {
      // tail mode: return last N lines
      slice = filtered.slice(-Math.min(lines, 200));
      nextOffset = total;
    }
    const clean = slice.map((l) => (l.length > 300 ? l.slice(0, 300) + "..." : l));
    jsonReply(res, 200, { ok: true, lines: clean, total, nextOffset, wallet: wallet || "all" });
  } catch (err) {
    jsonReply(res, 500, { ok: false, error: err.message });
  }
}
...
async function handleAction(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "full":
        log("dashboard", `Full agent cycle (manage + screen) triggered from ${ip}`);
        {
          const manageResult = await agentLoop(
            "Review all open positions and decide whether to stay, close (stop-loss/take-profit/OOR), or redeploy",
            config.llm.maxSteps, [], "MANAGER");
          const screenResult = await agentLoop(
            "Screen for new liquidity pool candidates and evaluate deployment opportunities",
            config.llm.maxSteps, [], "SCREENER");
          result = { manage: manageResult, screen: screenResult };
        }
        break;

      default:
        return jsonReply(res, 400, { ok: false, error: `Unknown action: ${action}. Valid: screen, manage, close, claim, briefing, full` });
    }
